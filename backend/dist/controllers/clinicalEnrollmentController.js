...
import { adminAddStudentToClinicalSlot, adminDropClinicalEnrollmentForSlot, dropStudentClinicalEnrollment, enrollStudentInClinicalSlot, listAdminClinicalSlotRoster, listOpenClinicalSlotsForStudent, listStudentClinicalEnrollmentRows, } from "../services/clinicalEnrollmentService.js";
import { setAdminClinicalEnrollmentGrade } from "../services/adminMarksService.js";
import { getStudentPortalClinicalBookingHold, runClinicalBookingPaymentHoldCleanup, } from "../services/clinicalBookingPaymentHoldService.js";
...
function parseOptionalGrade2(value) {
 ...
 */
export async function postAdminClinicalSlotEnrollmentGradeHandler(req, res) {
    try {
        const tid = pathTimetableId(req);
        if (!Number.isInteger(tid) || tid <= 0) {
            res.status(400).json({ error: "Invalid timetable id" });
