...
export async function postStudentEnroll(req, res) {
    try {
        ...
        if (!parsed) {
            res.status(400).json({
                error: "Invalid body: require studentId, academic_term_id, and sections[{ course_code, section_code, schedule_track? }]. schedule_track must be EN or CN when provided.",
            });
            return;
        }
...
export async function postStudentWithdraw(req, res) {
    try {
        ...
        if (parsed == null) {
            res.status(400).json({
                error: "Request body must include studentId, academic_term_id, and course_section_id (numeric course_sections.id).",
            });
            return;
        }
