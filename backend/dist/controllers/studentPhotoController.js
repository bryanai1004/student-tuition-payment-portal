...
import { STUDENT_PHOTO_ALLOWED_MIME_TYPES, STUDENT_PHOTO_MAX_SIZE_BYTES, StudentPhotoServiceError, getStudentPhotoUrl, uploadStudentPhotoForStudentId, } from "../services/adminStudentPhotoService.js";
const STUDENT_PHOTO_UPLOAD = multer({
    ...
    fileFilter: (_req, file, cb) => {
        if (STUDENT_PHOTO_ALLOWED_MIME_TYPES.includes(file.mimetype)) {
            cb(null, true);
            return;
        }
...
export function uploadStudentMyPhotoMiddleware(req, res, next) {
...
export async function getStudentMyPhotoUrlHandler(req, res) {
...
export async function postStudentMyPhotoHandler(req, res) {
    const studentId = getAuthenticatedStudentId(req);
    if (!studentId) {
        res.status(401).json({ success: false, message: "Authentication required." });
        return;
    ...
    if (!res.locals.photoUploadReady) {
        const message = typeof res.locals.photoUploadError === "string" &&
...
//# sourceMappingURL=studentPhotoController.js.map