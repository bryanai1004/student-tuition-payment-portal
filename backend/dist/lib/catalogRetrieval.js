...
export function normalizeCatalogQueryText(raw) {
    return raw
        .normalize("NFKC")
        .replace(/\s+/g, " ")
        .replace(/\s+([,.;:!?，。；：])/g, "$1")
 ...
 */
export function expandCatalogQueryForEmbedding(question, rewritten) {
    const parts = new Set();
    ...
    if (/临床|诊\s*所|见习|实习|时数|学时|小时/.test(bundle)) {
        add("clinical training clinical hours clerkship internship practicum clinic hours requirements");
    }
...
export function detectCatalogProgramHint(text) {
    const u = text.toUpperCase();
    const hasMahm = /\bMAHM\b|\bM\.A\.H\.M\.|\bMASTER\s+OF\s+HERBAL\b|针灸硕士|草药硕士/.test(text + u);
    const hasDahm = /\bDAHM\b|\bD\.A\.H\.M\.|\bDOCTOR\s+OF\s+HERBAL\b|针灸博士/.test(text + u);
    if (hasMahm && !hasDahm)
        return "MAHM";
    if (hasDahm && !hasMahm)
        return "DAHM";
...
function programScoreMultiplier(chunk, hint) {
    if (!hint || !chunk.program)
        return 1;
    if (chunk.program === hint)
        return 1.14;
    return 0.9;
}
export function rankCatalogChunksByEmbeddingMaxWithHint(chunks, queryEmbeddings, programHint, cosineSimilarity) {
...
export function isWeakCatalogRetrieval(maxScore) {
    return maxScore < WEAK_RETRIEVAL_MAX_SCORE;
}
export { WEAK_RETRIEVAL_MAX_SCORE };
//# sourceMappingURL=catalogRetrieval.js.map