 ...
 */
export async function selectCourseNamesByCode(courseCode) {
    const [rows] = await pool.query(`SELECT chi_name, eng_name
     FROM courses
     WHERE TRIM(code) = ?
     LIMIT 1`, [courseCode]);
 ...
 */
export async function selectDistinctTimetableInstructorIdsForCourse(courseCode) {
    const [rows] = await pool.query(`SELECT DISTINCT TRIM(t.instructor_id) AS iid
     FROM (
     ...
     WHERE TRIM(t.course) = ?
       ...
       AND TRIM(t.instructor_id) <> ''`, [courseCode]);
 ...
 */
export async function selectDistinctMarksInstructorsForCourse(courseCode) {
    const [rows] = await pool.query(`SELECT TRIM(instructor) AS instr
     FROM marks
     WHERE TRIM(code) = ?
       AND instructor IS NOT NULL
       AND TRIM(instructor) <> ''
     GROUP BY TRIM(instructor)`, [courseCode]);
    if (!Array.isArray(rows))
        return [];
    const out = [];
    for (const row of rows) {
        const s = trimOrEmpty(row.instr);
        if (s !== "")
            out.push(s);
    }
    return out;
}
//# sourceMappingURL=adminCourseMetaRepository.js.map