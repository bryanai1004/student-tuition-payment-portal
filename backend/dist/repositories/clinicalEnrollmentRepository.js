...
function normalizeEnrollmentTerm(term) {
    return term.trim().slice(0, 20);
}
const CLINICAL_COURSE_TEMPLATE_BY_BUCKET = {
    "100": {
        levelBucket: "100",
        baseCode: "CL111",
...
function parseClinicalAttemptSuffix(code, baseCode) {
...
async function resolveStudentNameForClinic(conn, studentId) {
    const sid = studentId.trim();
    const [studentRows] = await conn.query(`SELECT TRIM(name) AS name
       FROM students
      WHERE TRIM(id) = TRIM(?)
      LIMIT 1`, [sid]);
    if (studentRows.length > 0) {
        const n = trimOrEmpty(studentRows[0].name);
        if (n !== "")
            return n;
    }
    return sid;
}
async function nextClinicalAttemptCodeForBase(conn, args) {
    const [rows] = await conn.query(`SELECT code
       FROM clinic
      WHERE id = ?
        AND term = ?
        AND year = ?
...
async function insertClinicAttemptRowForEnrollment(conn, args) {
    const studentName = await resolveStudentNameForClinic(conn, args.studentId);
    await conn.query(`INSERT INTO clinic (
...
async function deleteLatestUngradedClinicAttemptForDrop(conn, args) {
    const likePrefix = `${args.baseCode.trim().toUpperCase()}-%`;
    const [rows] = await conn.query(`SELECT code, grade
       FROM clinic
      WHERE TRIM(id) = TRIM(?)
        AND TRIM(term) = TRIM(?)
        AND year = ?
    ...
    if (bestCode == null) {
        const [fallbackRows] = await conn.query(`SELECT code, grade
         FROM clinic
        WHERE TRIM(id) = TRIM(?)
          AND TRIM(term) = TRIM(?)
          AND year = ?
          AND UPPER(TRIM(code)) LIKE ?
        FOR UPDATE`, [args.studentId.trim(), args.term.trim(), args.year, likePrefix]);
    ...
    if (bestCode == null)
        return;
    await conn.query(`DELETE FROM clinic
      WHERE TRIM(id) = TRIM(?)
        AND TRIM(term) = TRIM(?)
        AND year = ?
        AND UPPER(TRIM(code)) = UPPER(TRIM(?))
        AND TRIM(COALESCE(grade, '')) = ''
      LIMIT 1`, [args.studentId.trim(), args.term.trim(), args.year, bestCode]);
}
...
function slotLabelFromTimetableFields(r) {
    return buildClinicTimetableSlotLabel({
        weekday: r.weekday,
        timeFrom: formatClinicTimeHm(typeof r.time_from === "string" ? r.time_from : String(r.time_from ?? "")),
        timeTo: formatClinicTimeHm(typeof r.time_to === "string" ? r.time_to : String(r.time_to ?? "")),
        slot: r.slot,
...
export async function listStudentClinicalEnrollments(studentId, options) {
    ...
    const yearClause = y != null && Number.isFinite(y) ? " AND ce.year = ? " : "";
    const params = [sid];
    if (t !== "") {
        params.push(t);
    }
    ...
    ${termClause}
    ${yearClause}
    ORDER BY ce.year DESC, TRIM(ce.term) ASC, ce.id ASC`, params);
    return rows.map((raw) => {
        const row = raw;
 ...
 */
export async function lockStudentClinicalEnrollmentForSlot(conn, studentId, timetableId, term, year) {
    const sid = studentId.trim();
    const te = normalizeEnrollmentTerm(term);
    const [rows] = await conn.query(`SELECT id, TRIM(status) AS status
       FROM clinical_enrollments
      WHERE TRIM(student_id) = TRIM(?)
        AND timetable_id = ?
        AND TRIM(term) = ?
        AND year = ?
      LIMIT 1
      FOR UPDATE`, [sid, timetableId, te, year]);
    if (rows.length === 0) {
        return null;
    }
    const r = rows[0];
 ...
 */
export async function lockAndCountActiveClinicalEnrollmentsForSlot(conn, timetableId, term, year) {
    const te = normalizeEnrollmentTerm(term);
    const [rows] = await conn.query(`SELECT COUNT(*) AS c
       FROM clinical_enrollments
      WHERE timetable_id = ?
        AND TRIM(term) = ?
        AND year = ?
        AND LOWER(TRIM(status)) = 'enrolled'
      FOR UPDATE`, [timetableId, te, year]);
    const r = rows[0];
    return Math.max(0, Math.trunc(Number(r?.c ?? 0)));
}
export async function insertClinicalEnrollmentRow(conn, input) {
    const status = input.status != null && String(input.status).trim() !== ""
        ? String(input.status).trim().toLowerCase().slice(0, 20)
        : "enrolled";
    const sb = input.seatBucket === undefined
        ? undefined
        : input.seatBucket === null
            ? null
            : input.seatBucket;
    const [res] = await conn.query(sb === undefined
...
export async function updateClinicalEnrollmentStatusById(conn, enrollmentId, studentId, status) {
    const st = status.trim().toLowerCase().slice(0, 20);
    const [res] = await conn.query(`UPDATE clinical_enrollments
        SET status = ?
      WHERE id = ?
        AND TRIM(student_id) = TRIM(?)`, [st, enrollmentId, studentId.trim()]);
    return res.affectedRows;
}
export async function updateClinicalEnrollmentStatusAndSeatBucketById(conn, enrollmentId, studentId, status, seatBucket) {
    const st = status.trim().toLowerCase().slice(0, 20);
    const [res] = await conn.query(`UPDATE clinical_enrollments
        SET status = ?,
            seat_bucket = ?
      WHERE id = ?
        AND TRIM(student_id) = TRIM(?)`, [st, seatBucket, enrollmentId, studentId.trim()]);
    return res.affectedRows;
}
/**
 * Marks timetable-linked assignments for this student/slot as dropped (non-destructive).
 */
export async function markClinicalAssignmentsDroppedForStudentSlot(conn, studentId, timetableId, term, year) {
    const te = normalizeEnrollmentTerm(term);
    const [res] = await conn.query(`UPDATE clinical_assignments
        SET status = 'Dropped'
      WHERE TRIM(student_id) = TRIM(?)
        AND timetable_id = ?
        AND TRIM(IFNULL(term, '')) = ?
        AND year = ?
        AND LOWER(TRIM(status)) NOT IN ('dropped', 'cancelled')`, [studentId.trim(), timetableId, te, year]);
    return res.affectedRows;
}
export async function countActiveClinicalEnrollmentsForSlot(timetableId, term, year) {
    const te = normalizeEnrollmentTerm(term);
    const [rows] = await pool.query(`SELECT COUNT(*) AS c
       FROM clinical_enrollments
      WHERE timetable_id = ?
        AND TRIM(term) = ?
        AND year = ?
        AND LOWER(TRIM(status)) = 'enrolled'`, [timetableId, te, year]);
    const r = rows[0];
    return Math.max(0, Math.trunc(Number(r?.c ?? 0)));
}
function normalizeEnrollmentSeatBucket(v) {
...
function aggregateEnrolledBucketsFromLockedRows(rows) {
    const out = { n100: 0, n200: 0, n300: 0, nAll: 0 };
    for (const raw of rows) {
        const row = raw;
...
export async function createClinicalEnrollment(studentId, timetableId, term, year, requestedSeatBucket, insertAssignment) {
    ...
    try {
        ...
        const [ttRows] = await conn.query(`SELECT seqNum AS id,
              TRIM(term) AS term,
              year,
              TRIM(day) AS weekday,
              time_from,
        ...
        const [lockedEnr] = await conn.query(`SELECT id,
        ...
        if (existing == null) {
            ...
            const n = await updateClinicalEnrollmentStatusAndSeatBucketById(conn, existing.id, sid, "enrolled", bucketEnforced ? chosenBucket : null);
            if (n === 0) {
                await conn.rollback();
 ...
 */
export async function dropClinicalEnrollmentInConn(conn, studentId, enrollmentId) {
    const sid = studentId.trim();
    const [rows] = await conn.query(`SELECT id, timetable_id, TRIM(term) AS term, year, TRIM(status) AS status, seat_bucket
       FROM clinical_enrollments
      WHERE id = ?
        AND TRIM(student_id) = TRIM(?)
      LIMIT 1
      FOR UPDATE`, [enrollmentId, sid]);
    if (rows.length === 0) {
    ...
    const seatBucket = seatBucketRaw === "100" ||
        seatBucketRaw === "200" ||
        seatBucketRaw === "300" ||
        seatBucketRaw === "all"
        ? seatBucketRaw
        : null;
    const n = await updateClinicalEnrollmentStatusById(conn, enrollmentId, sid, "dropped");
    if (n === 0) {
        return { ok: false, error: "Could not drop enrollment." };
    }
    await markClinicalAssignmentsDroppedForStudentSlot(conn, sid, timetableId, term, year);
    const [ttRows] = await conn.query(`SELECT TRIM(day) AS weekday,
            time_from,
            time_to,
            TRIM(instructor) AS instructor,
            \`100Max\` AS cap_100,
...
export async function dropClinicalEnrollment(studentId, enrollmentId) {
    const sid = studentId.trim();
    const conn = await pool.getConnection();
    try {
        await conn.beginTransaction();
...
//# sourceMappingURL=clinicalEnrollmentRepository.js.map