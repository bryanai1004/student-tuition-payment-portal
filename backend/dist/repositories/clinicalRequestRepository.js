...
function mapRequestRow(r) {
    ...
    return {
        ...
        decided_by: row.decided_by == null || row.decided_by === ""
            ? null
...
export async function insertClinicalRequestRow(params) {
    ...
    const [res] = await pool.query(`INSERT INTO clinical_requests
      (student_id, timetable_id, term, year, status)
     VALUES (TRIM(?), ?, ?, ?, 'pending')`, [params.studentId, params.timetableId, term, params.year]);
    return Number(res.insertId);
}
export async function studentHasPendingClinicalRequestForTimetable(studentId, timetableId) {
    ...
    const [rows] = await pool.query(`SELECT 1 AS ok
      ...
      LIMIT 1`, [sid, timetableId]);
    return rows.length > 0;
}
export async function listClinicalRequestsForStudent(studentId) {
    const sid = studentId.trim();
    const [rows] = await pool.query(`SELECT cr.id, cr.student_id, cr.timetable_id, cr.term, cr.year, cr.status,
      ...
      ORDER BY cr.created_at DESC, cr.id DESC`, [sid]);
    return rows.map(mapRequestRow);
}
export async function listPendingClinicalRequestsForAdmin() {
    const [rows] = await pool.query(`SELECT cr.id, cr.student_id, cr.timetable_id, cr.term, cr.year, cr.status,
            cr.created_at, cr.decided_at, cr.decided_by,
            ct.day AS tt_day, ct.time_from AS tt_time_from, ct.time_to AS tt_time_to,
            ct.slot AS tt_slot, ct.instructor AS tt_instructor
...
export async function getClinicalRequestById(id) {
...
export async function getClinicalRequestByIdForUpdate(connection, id) {
    if (!Number.isFinite(id) || id <= 0) {
        return null;
    }
    const [rows] = await connection.query(`SELECT cr.id, cr.student_id, cr.timetable_id, cr.term, cr.year, cr.status,
            cr.created_at, cr.decided_at, cr.decided_by,
            ct.day AS tt_day, ct.time_from AS tt_time_from, ct.time_to AS tt_time_to,
            ct.slot AS tt_slot, ct.instructor AS tt_instructor
       FROM clinical_requests cr
...
export async function updateClinicalRequestDecision(connection, id, status, decidedBy) {
    const [res] = await connection.query(`UPDATE clinical_requests
        SET status = ?,
            decided_at = CURRENT_TIMESTAMP,
            decided_by = ?
      WHERE id = ?
        AND LOWER(TRIM(status)) = 'pending'`, [status, decidedBy, id]);
    return Number(res.affectedRows);
}
//# sourceMappingURL=clinicalRequestRepository.js.map