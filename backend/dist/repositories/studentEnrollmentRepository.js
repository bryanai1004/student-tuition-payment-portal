...
function normalizeNullableYear(raw) {
    if (raw == null || raw === "")
        return null;
    const n = Number(raw);
 ...
 */
export async function listPortalEnrollmentRosterBySectionId(sectionId) {
    ...
    LEFT JOIN students s
     ...
     ORDER BY
       ...
       TRIM(s.name) ASC,
       TRIM(e.student_external_id) ASC`, [sid]);
    return rows
        .map((row) => ({
...
export async function listPortalEnrollmentTermsForStudent(studentExternalId) {
    const sid = studentExternalId.trim();
    const [rows] = await pool.query(`SELECT
       TRIM(e.term) AS term,
       e.year AS year
...
export async function listPortalEnrollmentHistoryForStudentTerm(studentExternalId, term, year) {
    const sid = studentExternalId.trim();
    const trimmedTerm = term.trim();
    const sql = `
    SELECT
    ...
    ORDER BY
      ...
      e.id ASC
  `;
    const [rows] = await pool.query(sql, [sid, trimmedTerm, year]);
    return rows.map((row) => {
        const unitsRaw = row.units;
        return {
            courseCode: String(row.course_code ?? "").trim(),
            courseTitle: row.course_title == null ? null : String(row.course_title).trim() || null,
            section: row.section_code == null ? null : String(row.section_code).trim() || null,
            units: unitsRaw == null || unitsRaw === ""
                ? null
