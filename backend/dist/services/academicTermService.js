...
export function isAcademicTermName(v) {
    return typeof v === "string" && TERM_NAMES.includes(v);
}
export function isAcademicTermStatus(v) {
    return typeof v === "string" && STATUSES.includes(v);
}
export function quarterIndexForTermName(name) {
    return QUARTER[name];
}
export function canonicalAcademicTermId(year, termName) {
    return `${year}-${TERM_SUFFIX[termName]}`;
}
export function defaultTermLabel(termName, year) {
...
function assertValidSequenceNo(n) {
    if (!Number.isInteger(n) || n <= 0) {
        throw new Error("Invalid sequence_no");
    }
}
function toDetailRow(input) {
    return {
        ...input,
        quarter_index: quarterIndexForTermName(input.term_name),
    };
}
export async function listAllAcademicTerms() {
    return listAcademicTerms();
}
export async function listVisibleTermsForStudents(limit) {
    return listVisibleAcademicTerms(limit);
}
export async function listRecentVisibleTerms(limit = 3) {
...
export async function createAcademicTerm(input) {
    ...
    const term_label = input.term_label?.trim() ||
...
export async function updateAcademicTerm(id, patch) {
    ...
    const term_name = patch.term_name !== undefined ? patch.term_name : existing.term_name;
    ...
    const sequence_no = patch.sequence_no !== undefined
        ? patch.sequence_no
    ...
    else if (patch.year !== undefined ||
        patch.term_name !== undefined) {
        term_label = defaultTermLabel(term_name, year);
    }
    const is_visible = patch.is_visible !== undefined ? patch.is_visible : existing.is_visible;
    const row = toDetailRow({
        id: nextId,
        term_label,
        year,
        ...
        registration_open: patch.registration_open !== undefined
