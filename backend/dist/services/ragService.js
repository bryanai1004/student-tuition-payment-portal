...
function formatRetrievedChunkContextBlock(items) {
    if (items.length === 0) {
        return "No retrieved AMU handbook or policy excerpts were available.";
    }
    return items
...
AHMC AI Department
bingchen.li@wanpanel.ai`;
function answerAlreadyContainsSupportContacts(answer) {
    return (/lli@amu\.edu/i.test(answer) ||
 ...
 */
function shouldSuggestTechnicalContact(question, intent) {
    if (intent === "direct" || intent === "out_of_scope")
        return false;
    const t = question.trim();
    const l = t.toLowerCase();
    const distress = /\b(not\s+work|doesn'?t\s+work|don'?t\s+work|won'?t\s+work|unable\s+to|can'?t|cannot|error|broken|bug|glitch|fail|failed|issue|problem|stuck|no\s+response|not\s+respond|not\s+responding|slow|timeout|down)\b/i.test(l);
    const loginPortal = /\b(log\s*in|sign\s*in|sign-in|password|locked\s+out|forgot\s+password|reset\s+password|student\s+portal|school\s+portal|payment\s+portal|pay\s+portal|online\s+portal|the\s+portal)\b/i.test(l) && distress;
    const sitePage = /\b(website|web\s*site|webpage|web\s+page|browser)\b/i.test(l) &&
        distress;
    const httpError = /\b(404|500|502|503)\b/.test(l) &&
        /\b(error|page|site|server|status|http)\b/i.test(l);
    const uploadAttach = /\b(upload|uploading|attachment|attached\s+file|file\s+upload)\b/i.test(l) && distress;
    const aiAssistant = /\b(ai\s+assistant|this\s+assistant|this\s+chat|chatbot|chat\s+bot)\b/i.test(l) && distress;
    const zhTech = /无法登录|登不(了|进去)|密码忘了|密码错误|上传失败|附件.{0,6}(失败|错误|传不)|系统.{0,8}(故障|崩溃|打不开)|页面.{0,6}(报错|打不开|空白)|助手.{0,8}(没反应|不回答)/.test(t);
    return (loginPortal ||
        sitePage ||
        httpError ||
        uploadAttach ||
...
function hasStrongAcademicAdvisingCue(trimmed, lower) {
    ...
    if (/\b(official\s+confirmation|confirm\s+with|verify\s+with|must\s+i\s+get\s+approval|need\s+approval|written\s+approval|prerequisite\s+waiver|waive\s+a\s+prerequisite|prereq\s+waiver)\b/i.test(lower)) {
        return true;
    }
    if (/\b(unclear\s+prerequisite|prerequisite\s+ambigu|prerequisite\s+unclear|which\s+prerequisite|prereq\s+conflict)\b/i.test(lower)) {
        return true;
    }
    if (/\b(prerequisite|prereq)\b/i.test(lower) &&
        /\b(unclear|ambigu|unsure|uncertain|confus|not\s+sure)\b/i.test(lower)) {
        return true;
    }
    if (/\b(transfer\s+credit|transcript\s+evaluat|transferring\s+in|will\s+my\s+credits)\b/i.test(lower)) {
        return true;
    }
    if (/\b(am\s+i\s+on\s+track|on\s+track\s+to\s+graduate|when\s+will\s+i\s+graduate|graduation\s+timeline|time\s+to\s+graduate|graduation\s+date\s+confirmation)\b/i.test(lower)) {
        return true;
    }
    if (/\b(eligib(le|ility)\s+for\s+the\s+program|program\s+eligib|admiss(?:ion|ions)\s+decision|background\s+fit|fit\s+for\s+the\s+program|will\s+i\s+be\s+accepted)\b/i.test(lower)) {
        return true;
    }
    if (/正式确认|官方确认|学术例外|豁免|申诉|休学|复学|转学学分|成绩评估|先修.{0,10}(不清|不明|模糊|冲突)|能否按时毕业|毕业时间.{0,6}确认|入学资格|录取.{0,6}(决定|结果)|背景.{0,8}是否符合/.test(trimmed)) {
        return true;
    }
    const wantsConfirmation = /\b(officially|official\s+ok|is\s+it\s+allowed|is\s+this\s+allowed|permitted\s+to|get\s+permission)\b/i.test(lower) || /正式允许|学校批准|书面批准|能不能选|可不可以选/.test(trimmed);
    const planning = /\b(course\s+plan|plan\s+my\s+courses|which\s+courses\s+should|class\s+schedule|course\s+sequence|semester\s+plan)\b/i.test(lower) || /选课规划|课程安排|怎么选课|先修顺序/.test(trimmed);
    return wantsConfirmation && planning;
}
/** Admissions / eligibility / prerequisite angle on the support-guidance path. */
function hasAcademicEscalationInSupportQuestion(trimmed, lower) {
    return (/\b(eligib|eligible|eligibility|admiss|applicant|apply\s+to|application|background|prior\s+degree|undergraduate\s+major|non[- ]traditional|prereq|prerequisite|transfer|graduate\s+on\s+time|graduation\s+path)\b/i.test(lower) ||
...
function supportQuestionNeedsAcademicContact(trimmed, lower) {
    if (isPurePaymentOperationalCue(trimmed, lower))
        return false;
 ...
 */
function isPurePaymentOperationalCue(trimmed, lower) {
    const paymentish = /\b(pay|payment|tuition|installment|refund|due\s+date|invoice|balance|late\s+fee)\b/i.test(lower) || /学费|支付|分期|退款|缴费|滞纳/.test(trimmed);
    if (!paymentish)
        return false;
    return !hasAcademicEscalationInSupportQuestion(trimmed, lower);
}
...
function appendSupportContactBlocks(answer, question, intent, guidanceSubtype) {
    if (answerAlreadyContainsSupportContacts(answer))
        return answer;
...
function detectLocalSearchCuisine(question) {
    const patterns = [
        {
...
function detectLocalSearchLocation(question) {
    const knownLocationMatches = Array.from(question.matchAll(/Los Angeles|Alhambra|Irvine|Pasadena|San Gabriel|Monterey Park|Arcadia|Rowland Heights|Anaheim|Orange County|洛杉矶|洛杉磯|阿罕布拉|尔湾|爾灣|帕萨迪纳|帕薩迪納|圣盖博|聖蓋博|蒙特利公园|蒙特利公園|亚凯迪亚|亞凱迪亞|橙县|橙縣/gi));
    const lastKnownLocation = knownLocationMatches.at(-1)?.[0];
    if (lastKnownLocation)
        return lastKnownLocation.trim();
    const englishAreaMatch = question.match(/\b(?:near|around|in)\s+([A-Za-z][A-Za-z\s-]{1,30})/i);
    if (englishAreaMatch?.[1]) {
        return englishAreaMatch[1].trim().replace(/\s+(for|with|that|which)$/i, "");
    }
    return null;
}
function buildLocalSearchKeywordSuggestions(question, zh) {
...
function buildLocalSearchReferenceLine(question, zh) {
    const cuisine = detectLocalSearchCuisine(question);
...
export async function answerEvidenceDrivenQuestion(input) {
...
export async function retrieveCatalogEvidenceForQuestion(question, rawHistory) {
    const q = validateQuestion(question);
    const history = sanitizeChatHistory(rawHistory);
    const client = getOpenAiClient();
...
export async function answerAmuQuestion(question, rawHistory, options) {
    ...
    const retrieval = hasProvidedCatalogEvidence
        ? null
    ...
    const weakRetrieval = options?.weakRetrieval ??
