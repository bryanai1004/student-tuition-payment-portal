...
function hasPersonalReferenceCue(value) {
    return (/\b(i|me|my|mine|am i|do i|did i|have i|for me)\b/i.test(value) ||
        /我|我的|我现在|我目前|我当前/.test(value));
}
function hasStudentRecordTopicCue(value) {
    return (/\b(course|courses|class|classes|enroll|enrolled|enrollment|taking|credits?|grade|grades|term|terms|record|records|schedule|withdraw|withdrawn|completed|transcript)\b/i.test(value) ||
        /选课|选了|修了|上了|课程|学分|成绩|注册|退选|退课|学期|记录|课表|在读|已修/.test(value));
}
function hasPolicyCue(value) {
...
function isConversationalWritingAssistOnly(question) {
    ...
    if (t.length > 800)
        return false;
    const assist = /\b(rewrite|rephrase|polish|proofread|translate|summarize|fix my grammar|make this sound)\b/i.test(t) || /润色|改写|翻译|总结|措辞|优化一下|改成更|帮我改|修一下这段|把下面/.test(t);
    if (!assist)
        return false;
    if (/\b(catalog|tuition|withdraw|refund|graduation|enrollment|clinical|credit|policy|deadline)\b/i.test(t) ||
...
function hasLocalSearchCue(value) {
    const explicitPhraseCue = /\b(near me|nearby|around here|close by|good places near|best places near|recommend places|recommend restaurants|restaurant recommendations|food recommendations)\b/i.test(value) ||
        /哪里好吃|哪裡好吃|附近有什么|附近有什麼|附近有啥|附近吃什么|附近吃什麼|附近有什么好吃|附近有什麼好吃|推荐餐厅|推薦餐廳|推荐美食|推薦美食|附近有什么地方可以去|附近有什麼地方可以去/.test(value);
    const placeTopicCue = /\b(restaurant|restaurants|food|eat|eating|lunch|dinner|brunch|breakfast|coffee|cafe|cafes|boba|milk tea|dessert|hot pot|bbq|ramen|sushi|tacos?|burger|pizza|places|spots|things to do|attractions?)\b/i.test(value) ||
        /餐厅|餐廳|饭店|美食|吃的|吃饭|吃飯|火锅|火鍋|奶茶|咖啡|甜品|烧烤|燒烤|拉面|拉麵|寿司|壽司|早午餐|景点|景點|玩的地方/.test(value);
    const recommendationCue = /\b(recommend|recommendation|suggest|suggestion|good|best|favorite|favourite|worth trying)\b/i.test(value) ||
        /推荐|推薦|好吃|值得去|值得吃|有什么好|有什麼好/.test(value);
    const areaCue = /\b(near|nearby|around|in)\s+[a-z][a-z\s-]{1,40}\b/i.test(value) ||
        /\b(los angeles|alhambra|irvine|pasadena|san gabriel|monterey park|arcadia|rowland heights|anaheim|orange county)\b/i.test(value) ||
        /附近|周边|周邊|洛杉矶|洛杉磯|阿罕布拉|尔湾|爾灣|帕萨迪纳|帕薩迪納|圣盖博|聖蓋博|蒙特利公园|蒙特利公園|亚凯迪亚|亞凱迪亞|橙县|橙縣/.test(value);
...
export function detectStudentRecordQuestion(question) {
    ...
    if (/\bhow\s+many\s+(courses|classes)\s+(am i|i am|i'm)\s+(taking|enrolled in)\b/i.test(normalized) ||
        /\bhow\s+many\s+(courses|classes)\s+am\s+i\s+taking\s+(this term|now|currently)\b/i.test(normalized) ||
        /我(现在|目前|当前).{0,8}(选|修|上).{0,4}了?.{0,4}多少(门|节)?课/.test(normalized)) {
        return { kind: "current_term_course_count" };
    }
    if (/\bhow\s+many\s+credits\s+(am i|i am|i'm)\s+(taking|enrolled in)\b/i.test(normalized) ||
        /\bhow\s+many\s+credits\s+am\s+i\s+taking\s+(this term|now|currently)\b/i.test(normalized) ||
    ...
    if (/\bhow\s+many\s+terms\s+(have i|i have|i've)\s+(registered|enrolled)\b/i.test(normalized) ||
    ...
    if (/\b(what|which)\s+(courses|classes)\s+(have i|i have|i've|did i)\s+(take|taken|took|completed|finished|studied)\b/i.test(normalized) ||
        ...
        /\b(show|list)\s+my\s+(courses|classes|academic history|record|records|transcript)\b/i.test(normalized) ||
        /我.{0,8}(修过|上过|学过|选过|完成过).{0,8}(什么课|哪些课|哪些课程)/.test(normalized) ||
    ...
    if (/\bhow\s+many\s+credits\s+do\s+i\s+have\b/i.test(normalized) ||
        /\bhow\s+many\s+credits\s+(have i|i have|i've)\s+completed\b/i.test(normalized) ||
        /\bhow\s+many\s+earned\s+credits\s+do\s+i\s+have\b/i.test(normalized) ||
        /我.{0,6}(总共|一共|已经)?.{0,6}(有|拿了|获得了|修了|完成了)?.{0,6}多少学分/.test(normalized) ||
...
export function detectCourseEligibilityIntent(question) {
    const normalized = lower(question);
    return (/\b(can\s+i\s+take|am\s+i\s+eligible|eligible\s+for|prerequisite|prereq|co-?requisite|what\s+am\s+i\s+missing)\b/i.test(normalized) ||
        /我可以选|我可不可以选|我能选|我还差什么课|先修|先决|能不能修|是否满足先修/.test(normalized));
}
export function classifyStudentAiIntent(question) {
    ...
    const personalAcademicCue = recordMatch != null ||
    ...
    const courseCode = extractCourseCode(question);
    const canITakeCourse = courseCode != null &&
        /\bcan\s+i\s+take\b/i.test(normalized) &&
        /\b(next term|next semester|this term|now|currently|later)\b/i.test(normalized);
    const canITakeCourseZh = /我.{0,12}(现在|这学期|下学期|目前)?.{0,10}(能不能|可不可以|能否|可以|能).{0,10}(选|修)/.test(normalized);
    if (canITakeCourse ||
        canITakeCourseZh ||
...
function hasCatalogSemanticCue(value) {
    return (/\b(amu|catalog|program|graduation|degree|credits?|credit load|tuition|fees?|prerequisite|requirement|requirements|policy|curriculum)\b/i.test(value) ||
        /AMU|目录|目錄|项目|項目|毕业|畢業|学分|學分|学费|學費|费用|費用|先修|先決|要求|政策|课程要求|課程要求/.test(value));
}
function hasPersonalStudentCue(value) {
    return (/\b(i|me|my|mine|am i|do i|did i|have i|for me|my transcript|my grades|my credits|my eligibility|what am i missing)\b/i.test(value) ||
        /我|我的|我现在|我目前|我当前|我还差|我還差|成绩单|成績單|我的成绩|我的成績|我的学分|我的學分|我的资格|我的資格|我缺什么|我缺什麼/.test(value));
}
export function needsCatalogEvidence(question) {
    const normalized = lower(question);
    return hasCatalogSemanticCue(normalized) || detectCourseEligibilityIntent(question);
}
export function needsStudentEvidence(question) {
    const normalized = lower(question);
