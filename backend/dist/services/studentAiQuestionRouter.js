...
function hasPolicyCue(value) {
    return (/\b(policy|handbook|catalog|rule|rules|requirement|requirements|deadline|deadlines|prerequisite|prerequisites|graduation|attendance|probation|refund|withdrawal policy|withdrawal|add\/drop|registration policy|tuition|fee|fees|payment)\b/i.test(value) ||
        /政策|规定|要求|规则|退选|退课|先修|先决|学费|费用|缴费|退款|毕业要求|目录|手册|deadline|withdrawal|prerequisite/.test(value));
}
...
export function classifyStudentAiIntent(question) {
    ...
    const canITakeCourseZh = courseCode != null &&
        /我.{0,8}(现在|这学期|下学期).{0,8}(能不能|可不可以|能否).{0,8}(选|修)/.test(normalized);
    if (canITakeCourse ||
        canITakeCourseZh ||
