...
export class StudentDocumentsValidationError extends Error {
    name = "StudentDocumentsValidationError";
    constructor(message) {
        super(message);
    }
}
export class StudentDocumentsNotFoundError extends Error {
    name = "StudentDocumentsNotFoundError";
    constructor(message) {
        super(message);
    }
}
...
function requirementOrderIndex(t) {
    const i = DOCUMENT_REQUIREMENT_TYPES.indexOf(t);
    return i === -1 ? 99 : i;
}
function sortByRequirementType(rows) {
    return [...rows].sort((a, b) => requirementOrderIndex(a.requirementType) -
...
async function assertAcademicTermExists(academicTermId) {
...
function assignmentFieldsFromExisting(existing) {
    if (!existing) {
        return {
            assignedBy: null,
...
export async function listStudentDocumentRequirementsForTerm(studentExternalId, academicTermId) {
...
export async function getAdminStudentDocumentRequirements(studentExternalId, academicTermId) {
    return listStudentDocumentRequirementsForTerm(studentExternalId, academicTermId);
}
export async function submitStudentAgreement(studentExternalId, academicTermId) {
    const sid = normId(studentExternalId);
    const tid = normId(academicTermId);
    if (!sid) {
        throw new StudentDocumentsValidationError("Missing student id");
    ...
    await runInTransaction(async (db) => {
        ...
        const attemptNo = await getNextDocumentAttemptNumber(db, sid, tid, "copyright_release_agreement");
...
export async function submitStudentQuizAttempt(studentExternalId, academicTermId, quizId, answers) {
    const sid = normId(studentExternalId);
    const tid = normId(academicTermId);
    if (!sid) {
        throw new StudentDocumentsValidationError("Missing student id");
...
export async function resetAdminStudentDocumentRequirementsForTerm(studentExternalId, academicTermId, reassignedBy) {
    const sid = normId(studentExternalId);
    const tid = normId(academicTermId);
    if (!sid) {
        throw new StudentDocumentsValidationError("Missing student id");
...
//# sourceMappingURL=studentDocumentsService.js.map