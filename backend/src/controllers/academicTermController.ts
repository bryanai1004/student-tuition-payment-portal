...
function pathTermId(req: Request): string | null {
...
function parseSequenceNo(v: unknown): number | null {
  if (v === undefined || v === null) return null;
  const n = typeof v === "number" ? v : Number(v);
  if (!Number.isFinite(n)) return null;
  return Math.trunc(n);
...
function parseCreateBody(body: unknown): CreateAcademicTermInput | null {
  if (!body || typeof body !== "object") return null;
  const o = body as Record<string, unknown>;
  const year = parseYear(o.year);
  const sequence_no = parseSequenceNo(o.sequence_no);
  if (year === null || sequence_no === null) return null;
  const term_name = o.term_name;
  if (!isAcademicTermName(term_name)) return null;
  const status = o.status;
  if (!isAcademicTermStatus(status)) return null;

  const term_label =
    typeof o.term_label === "string" ? o.term_label.trim() : undefined;
...
function parsePatchBody(body: unknown): UpdateAcademicTermInput | null {
  if (!body || typeof body !== "object") return null;
  const o = body as Record<string, unknown>;
  const patch: UpdateAcademicTermInput = {};
  ...
  if (o.term_name !== undefined) {
    if (!isAcademicTermName(o.term_name)) return null;
    patch.term_name = o.term_name;
  }
  if (o.status !== undefined) {
    if (!isAcademicTermStatus(o.status)) return null;
    patch.status = o.status;
  }
  if (o.term_label !== undefined) {
    if (typeof o.term_label !== "string") return null;
...
function patchHasField(patch: UpdateAcademicTermInput): boolean {
...
export async function getAcademicTermsRecent(
  req: Request,
  res: Response,
): Promise<void> {
  try {
...
export async function getAcademicTermsCurrent(
  _req: Request,
  res: Response,
): Promise<void> {
  try {
...
export async function postAdminAcademicTerm(
  ...
  try {
    ...
    if (!input) {
      res.status(400).json({
        error:
          "Invalid body: require year, term_name, sequence_no, status; optional date fields (YYYY-MM-DD), term_label, is_visible, payment_due_date, lock_registration_if_overdue",
      });
      return;
    }
...
export async function patchAdminAcademicTerm(
  req: Request,
  res: Response,
): Promise<void> {
  try {
