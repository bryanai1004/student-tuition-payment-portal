...
import {
  AdminClinicalSlotError,
  createAdminClinicalSlot,
  deleteAdminClinicalSlot,
  listAdminClinicalSlots,
  updateAdminClinicalSlot,
  type AdminClinicalSlotCreateInput,
  type AdminClinicalSlotPatchInput,
} from "../services/adminClinicalSlotService.js";
...
function pathSlotId(req: Request): number | null {
...
function parseCreateBody(body: unknown): AdminClinicalSlotCreateInput | null {
  ...
  const academicTermId =
    typeof o.academicTermId === "string" ? o.academicTermId.trim() : "";
  ...
  const instructor =
    typeof o.instructor === "string" ? o.instructor : "TBA";
  ...
  return {
    ...
    slot,
    instructorId:
      o.instructorId === undefined || o.instructorId === null
        ? ""
...
function parsePatchBody(body: unknown): AdminClinicalSlotPatchInput | null {
  if (!body || typeof body !== "object") return null;
  const o = body as Record<string, unknown>;
  const patch: AdminClinicalSlotPatchInput = {};
  ...
  if (Object.prototype.hasOwnProperty.call(o, "instructor")) {
    if (typeof o.instructor !== "string") return null;
    patch.instructor = o.instructor;
  }
  for (const key of ["cap100", "cap200", "cap300", "cap123"] as const) {
 ...
 */
export async function postAdminClinicalSlotHandler(
  req: Request,
  res: Response,
): Promise<void> {
  try {
 ...
 */
export async function patchAdminClinicalSlotHandler(
  req: Request,
  res: Response,
): Promise<void> {
  try {
 ...
 */
export async function deleteAdminClinicalSlotHandler(
  req: Request,
  res: Response,
): Promise<void> {
  try {
