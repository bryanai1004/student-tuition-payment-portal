 ...
 */
export async function getAdminExportRegisteredStudentsCsv(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    if (env.nodeEnv === "development" && built.devDiagnostic != null) {
      console.log(
        "[admin/course-sections/export-registered-students] diagnostic",
        {
          route:
            "GET /api/admin/course-sections/:id/export-registered-students.csv",
          sectionId: id,
    ...
    /** Confirms this response was generated by the per-question roster CSV path (curl -I to verify deployment). */
    res.setHeader("X-Portal-Roster-Csv-Schema", "course-feedback-per-question-v1");
