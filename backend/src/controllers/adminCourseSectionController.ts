...
function parseCreateBody(body: unknown): CourseSectionCreateInput | null {
  ...
  if (!course_code || !term || !section_code || !weekday) return null;
...
function parsePatchBody(body: unknown): CourseSectionUpdateInput | null {
  if (!body || typeof body !== "object") return null;
  const o = body as Record<string, unknown>;
  const patch: CourseSectionUpdateInput = {};
  if (typeof o.course_code === "string")
    patch.course_code = o.course_code.trim();
  if (typeof o.term === "string") patch.term = o.term.trim();
  ...
  if (o.start_time !== undefined) patch.start_time = strOrNull(o.start_time);
  if (o.end_time !== undefined) patch.end_time = strOrNull(o.end_time);
  if (o.delivery_mode !== undefined)
    patch.delivery_mode = strOrNull(o.delivery_mode);
  if (o.room !== undefined) patch.room = strOrNull(o.room);
  if (o.instructor !== undefined)
    patch.instructor = strOrNull(o.instructor);
  if (o.notes !== undefined) patch.notes = strOrNull(o.notes);
  return patch;
}
...
function patchHasUpdatableField(patch: CourseSectionUpdateInput): boolean {
...
export async function postAdminCourseSection(
  ...
  try {
    ...
    if (!input) {
      res.status(400).json({
        error:
          "Invalid body: require course_code, term, year, section_code, weekday",
      });
      return;
    }
