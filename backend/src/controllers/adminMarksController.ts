...
function parseSetGradeBody(
  body: unknown,
): { studentId: string; courseCode: string; academicTermId: string; grade: string } | null {
  ...
  if (studentId === "" || courseCode === "" || academicTermId === "") return null;
 ...
 */
export async function setStudentGrade(req: Request, res: Response): Promise<void> {
  try {
