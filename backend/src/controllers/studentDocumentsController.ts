...
import {
  getAdminStudentDocumentRequirements,
  listStudentDocumentRequirementsForTerm,
  resetAdminStudentDocumentRequirement,
  resetAdminStudentDocumentRequirementsForTerm,
  StudentDocumentsNotFoundError,
  StudentDocumentsValidationError,
  submitStudentAgreement,
  submitStudentQuizAttempt,
} from "../services/studentDocumentsService.js";
import {
...
function parseBodyAcademicTermId(body: unknown): string | null {
...
function parseBodyReassignedBy(body: unknown): string | null {
  if (!body || typeof body !== "object") return null;
  const o = body as Record<string, unknown>;
  if (!Object.prototype.hasOwnProperty.call(o, "reassignedBy")) return null;
...
function parseAgreementBody(body: unknown): { academicTermId: string } | null {
  const academicTermId = parseBodyAcademicTermId(body);
  if (!academicTermId) return null;
  return { academicTermId };
}

function parseQuizSubmitBody(
  body: unknown,
): { academicTermId: string; answers: Record<string, string> } | null {
  if (!body || typeof body !== "object") return null;
  const o = body as Record<string, unknown>;
  const academicTermId = parseBodyAcademicTermId(body);
  if (!academicTermId) return null;
  const answersRaw = o.answers;
  if (answersRaw === undefined || answersRaw === null) return null;
  if (typeof answersRaw !== "object" || Array.isArray(answersRaw)) return null;
...
function parseResetBody(
  body: unknown,
): { academicTermId: string; reassignedBy: string | null } | null {
  const academicTermId = parseBodyAcademicTermId(body);
  if (!academicTermId) return null;
...
export async function getStudentDocumentRequirementsHandler(
  req: Request,
  res: Response,
): Promise<void> {
  try {
...
export async function postStudentAgreementSubmitHandler(
  req: Request,
  res: Response,
): Promise<void> {
  try {
...
export async function postStudentQuizSubmitHandler(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    if (!parsed) {
      res.status(400).json({
        error:
          "Invalid body: require academicTermId (string) and answers (object mapping question id to string).",
      });
      return;
    }
    const payload = await submitStudentQuizAttempt(
      sid,
      parsed.academicTermId,
      quizId,
      parsed.answers,
    );
    res.status(200).json(payload);
...
export async function getAdminStudentDocumentRequirementsHandler(
  req: Request,
  res: Response,
): Promise<void> {
  try {
...
export async function postAdminStudentDocumentRequirementResetHandler(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    const payload = await resetAdminStudentDocumentRequirement(
      sid,
      parsed.academicTermId,
      rt,
      parsed.reassignedBy,
...
export async function postAdminStudentDocumentRequirementsResetAllHandler(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    const payload = await resetAdminStudentDocumentRequirementsForTerm(
      sid,
      parsed.academicTermId,
      parsed.reassignedBy,
    );
