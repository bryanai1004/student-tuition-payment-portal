...
function extractLikelyCourseNameToken(question: string): string | null {
  ...
  if (genericEnglishTokens.has(lowerCompact)) return null;
  if (
    /学分|毕业|学费|缴费|付款|payment|tuition|program|graduation|requirement|requirements|policy/i.test(
      compact,
    )
  ) {
    return null;
...
export function isLikelyCourseRelatedQuery(question: string): boolean {
  ...
  const isBroadNonCourseQuestion =
    /我(现在)?修了多少学分|my\s+credits|completed\s+credits|my\s+completed\s+credits|my\s+transcript|my\s+gpa|my\s+grades/i.test(
      lower,
    ) ||
    /毕业需要多少学分|graduation\s+requirement|program\s+credits|一个学分多少钱|我的学费是多少|payment\s+standard|tuition|payment|policy|program|graduation|学分多少钱|学费|缴费|付款/.test(
      q,
    );
  if (isBroadNonCourseQuestion) return false;
  ...
  if (hasIntent && hasLikelyCourseName) return true;
  const shortCourseLike = isShortCourseLikeQuery(q);
  if (shortCourseLike) return true;
  if (
