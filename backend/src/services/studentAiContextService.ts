...
export async function buildStudentAiContext(
  studentId: string,
): Promise<StudentAiContextBuildResult> {
  ...
  const currentEnrollments = academics.currentSchedule.slice(0, 12).map((item) => {
    ...
    if (item.status && item.status !== "active") details.push(`status: ${item.status}`);
    if (item.instructor) details.push(`instructor: ${item.instructor}`);
  ...
  const completedCourses = academics.transcript
    ...
    .map((item) =>
      formatCourseLine(
        item.courseCode,
        item.courseTitle,
        `${item.grade}${item.term && item.year ? `, ${item.term} ${item.year}` : ""}`,
      ),
    );
  ...
  if (currentEnrollments.length === 0) {
    ...
    for (const item of currentEnrollments) lines.push(`  - ${item}`);
  }
  ...
  if (recentHistory.length === 0) {
    ...
    for (const item of recentHistory) lines.push(`  - ${item}`);
  }
  ...
  if (completedCourses.length === 0) {
    ...
    for (const item of completedCourses) lines.push(`  - ${item}`);
  }
