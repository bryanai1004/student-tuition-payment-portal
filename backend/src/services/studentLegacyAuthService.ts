 ...
 */
export function deriveLastNameInitial(name: string): string | null {
  ...
  const firstToken =
    spaceIdx >= 0 ? trimmed.slice(0, spaceIdx).trim() : trimmed;
...
export function legacyPasswordIdSuffix(studentId: string): string {
  const id = studentId.trim();
  if (id.length < 5) return id;
  return id.slice(-5);
}

export function buildExpectedLegacyPassword(
  studentName: string,
  studentId: string,
): string | null {
