...
export function readStoredGeometryClamped(
  ...
  if (!parsed) return defaultGeometry(viewportWidth, viewportHeight)
  return clampGeometry(parsed, viewportWidth, viewportHeight)
}

export function persistGeometry(g: AIAssistantStoredGeometry): void {
  try {
    localStorage.setItem(AI_ASSISTANT_STORAGE_KEY, JSON.stringify(g))
  } catch {
