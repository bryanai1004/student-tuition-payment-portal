...
import { requestStudentPasswordReset } from '../lib/api'
...
export function ForgotPasswordPage() {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true)
    try {
      const result = await requestStudentPasswordReset(trimmed)
      setFormNotice(result.message || t('resetLinkSent'))
    } catch (e) {
      ...
      <div className="portal-login-body">
        <div className="portal-login-stack">
          ...
          <article className="portal-login-card">
            <h1 className="portal-login-card-title">{t('forgotPasswordTitle')}</h1>
            <p className="portal-login-card-intro">{t('forgotPasswordIntro')}</p>

            <div className="portal-login-fields">
              <div className="portal-login-field">
                <label className="portal-login-label" htmlFor="forgot-login-email">
                  {t('loginEmail')}
                </label>
                <input
